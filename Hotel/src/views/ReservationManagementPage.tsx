      ...
      <div className="w-full max-w-[1280px] min-h-[calc(100vh-120px)] lg:h-[850px] bg-[var(--main-card)] rounded-xl shadow-[var(--card-shadow)] flex flex-col overflow-hidden border border-[var(--main-border)] transition-colors duration-300">
        ...
        <div className="flex flex-col lg:grid lg:grid-cols-[1fr_420px] flex-grow overflow-hidden">
          ...
          <div className="p-4 sm:p-6 md:p-10 lg:overflow-y-auto border-b lg:border-b-0 lg:border-r border-[var(--main-border)] bg-[var(--main-card)] flex flex-col min-h-[400px]">
            ...
            {/* NUEVA ZONA DE FILTROS ACTUALIZADA */}
            <div className="flex flex-col sm:flex-row gap-4 mb-6">
              ...
              {/* Filtro de Espacio (Habitación / Salón) */}
              <select 
                value={filtros.tipo}
                onChange={(e) => setFiltros({...filtros, tipo: e.target.value})}
                className="input-primary !py-2.5 cursor-pointer w-full sm:w-auto"
              >
                <option value="">Todos los espacios</option>
                <option value="habitacion">Solo Habitaciones</option>
                <option value="salon_eventos">Solo Salón de Eventos</option>
              </select>
                ...
                <table className="w-full text-left border-collapse min-w-[600px]">
                  <thead>
                    <tr className="border-b-2 border-[var(--main-border)]">
                      <th className="py-3 px-2 text-xs text-[var(--text-muted)] uppercase font-semibold">ID / Espacio</th>
                        ...
                        >
                          <td className="py-4 px-2">
                            ...
                            <div className="flex flex-col mt-0.5">
                              <span className="text-xs text-[var(--text-muted)]">Hab. {res.habitacionId}</span>
                              {/* NUEVO: Etiqueta visual para distinguir Salón vs Habitación rápidamente */}
                              <span className={`text-[9px] uppercase font-bold tracking-wider mt-0.5 ${res.tipoHabitacion === 'salon_eventos' ? 'text-[var(--brand-yellow-hover)]' : 'text-[var(--text-muted)] opacity-70'}`}>
                                {res.tipoHabitacion === 'salon_eventos' ? '★ Salón de Eventos' : res.tipoHabitacion}
                              </span>
                            </div>
                          </td>
                          ...
                          <td className="py-4 px-4 text-xs text-[var(--text-main)]">
                            ...
                            {/* Si es salón de eventos, no hace falta pintar la fecha de salida porque es la misma */}
